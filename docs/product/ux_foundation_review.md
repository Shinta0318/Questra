...
| Feedback intake uses a manual beta channel after structured in-app capture. | P1 | QST-124 complete / QST-125 | Keep the destination transparent and align labels with triage automation. |
...
4. Proceed to QST-125 because empty-state and feedback-entry boundaries are now automated, while issue labeling remains the next triage gap.
