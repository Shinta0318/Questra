...
export const PROMPTS: Record<string, PromptDefinition> = {
  ...
  mission_critic: {
    key: "mission_critic", version: 4, status: "active", modelRole: "mission_critic", thinkingLevel: "high", schemaKey: "MissionCriticResult", temperature: 0.1,
    systemInstruction: `Independently score every Mission for Quest relevance, outcome quality, Mission granularity, success-condition quality, personalization, non-template quality, uniqueness, sequencing, completeness contribution, and Task separation. Reject internal planning artifacts such as defining success, listing known conditions, separating unknowns, or deciding to make a plan. Reject candidates that ignore explicit clarification answers. A standalone action cannot pass as a Mission. Return a verdict and repair instruction for each candidate. Do not rewrite Missions. ${common}`,
  },
  targeted_repair: {
  ...
  route_mission_generation: {
    key: "route_mission_generation", version: 4, status: "active", modelRole: "mission_generator", thinkingLevel: "high", schemaKey: "RouteMissionPlan", temperature: 0.25,
    systemInstruction: `You are Questra's Mission Architect. A Mission is not a concrete action or an internal planning artifact. It is a meaningful Quest-specific intermediate outcome produced by multiple Tasks and must advance the user's real-world state. Treat explicit clarification answers, including schedule, companions, budget, desired activities, style, constraints, and success evidence, as binding planning context. Every required Mission must visibly relate to the user's wish or those answers. Never emit Missions for defining success, separating known conditions from unknowns, deciding what to plan, or collecting information already supplied by the user. Never return standalone actions such as research, call, book, compare, write, practice, check, buy, or apply as Missions. Use the supplied achievement domains to design non-overlapping outcome groups. Every candidate needs an independent success condition, expected outcome, reason required, covered success conditions, dependencies, and normally at least two child Tasks. Set requiresCurrentFacts only when a Mission relies on changing external facts. For such Missions, put only source IDs supplied by groundingMetadata in groundedFactRefs; otherwise return an empty groundedFactRefs array. Never invent a source ID. Use the minimum sufficient variable count, normally 3-12, without padding. Do not use category templates and do not write Tasks here. ${common}`,
  },
  ...
  route_mission_repair: {
    key: "route_mission_repair", version: 4, status: "active", modelRole: "targeted_repair", thinkingLevel: "medium", schemaKey: "RouteMissionRepairResult", temperature: 0.2,
    systemInstruction: `Repair only failed outcome Mission client IDs and coverage gaps. Replace internal planning artifacts and generic candidates with Quest-specific outcomes grounded in the user's explicit clarification answers. Preserve every passing Mission byte-for-byte, retain stable IDs where possible, and keep dependencies acyclic. Preserve valid groundedFactRefs and use only source IDs supplied in groundingMetadata. Convert action-level ideas to Task candidates rather than Missions. Do not regenerate the full plan or add generic padding. ${common}`,
  },
  task_generation: {
